...
{
    public class MailNaultinusViewModel : ViewModelBase
    {
        private readonly MailNaultinusModel _model;
        ...
        public MailNaultinusViewModel() : this(new MailNaultinusModel { Name = Strings.MailDefaultName, Width = 320, Height = 240 })
        { }

        public MailNaultinusViewModel(MailNaultinusModel model) : base(model)
        {
            _model = model;
        ...
        private async Task EnsureConnectedAsync()
        {
            if (_mailService == null)
                _mailService = NaultinusFactory.CreateImapMailService(_model);
            try
            {
        ...
        public async Task RefreshAsync()
        {
            ...
            try
            {
                ...
                foreach (var folder in _model.MonitoredFolders ?? new List<string> { "INBOX" })
                {
                    ...
                    {
                        NaultinusDiagnostics.Log("MailNaultinus", "Lecture du nombre de messages impossible pour " + folder, ex);
                        counts[folder] = 0;
                    }
                }
                lock (_countsLock)
        ...
        public void OpenWebmail()
        {
            ...
            catch (Exception ex) { NaultinusDiagnostics.Log("MailNaultinus", "Ouverture du webmail impossible : " + url, ex); }
        }

        #region Commands

        public ICommand RefreshCommand { get; } = new AsyncRelayCommand<MailNaultinusViewModel>(async vm => { if (vm != null) await vm.RefreshAsync(); });
        public ICommand OpenWebmailCommand { get; } = new RelayCommand<MailNaultinusViewModel>(vm => { vm?.OpenWebmail(); });
